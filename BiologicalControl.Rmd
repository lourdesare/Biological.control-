...
In these same two cultivars the biomass distribution by fractions was measured, in order to measure the plant grow capacity, and they were divided in tree fraction categories: small (less than 35 g), medium (36–180 g), or large (more than 180 g). Yields were determined by weighing all daughter tubers from each plot in the both years. For statistical tests, all data were analyzed performing Duncan and Tukey tests.
...
Also in this review, the aim of this group is to compare the analysis done in the article between the Duncan, the Kruskal Wallis and Turkey tests to understand why Duncan was used in the article.


## Hypothesis

*B. velezensis* strains can be used as biological control agents against *R. solani*, and increase significantly the plant growth of potato. Moreover, there is no significant difference between the results obtained through the Duncan, Kruskal Wallis and Turkey test.
...
We can see only few differences between the results of Duncan, Tukey and Kruskal test, in the differences between treatments (Figures 1, 2 and 3). Also, the Shapiro-Wilk normality test was performed for Anova and Square Root Anova because in Anova p was almost higher than 0.05, that means that the data is really similar to a normal distribution (Figure 4) and, in this case, the best test to be used would be the Tukey test because of the normal distribution of the results.

Taking into account the Tukey test for the plant height, we can see that the values increase over the weeks, however the difference between the treatments is not so great and in most cases the first treatment (BZR 336g) generates the highest plant height and the second (BZR 517) is an intermediate value, when compared to the control. Moreover, this difference is most noticeable in 10 weeks after planting when only the cultivar Svitanok kievski in 2016 had no significant difference between treatments. 
...
We can see only few differences between the results of Duncan, Tukey and Kruskal test, in the differences between treatments (Figures 5, 6 and 7). Also, The Shapiro-Wilk normality test was performed for Anova and Square Root Anova because in Anova p was almost higher than 0.05 (in the Anova, with the normal values), what means that the data is really similar to a normal distribution (Figure 8) and, in this case the best test to be used would be the Tukey test because of the normal distribution of the results.

Taking into account the Tukey test for disease incidence, we can see a big difference between the control and the treatments principally 10 weeks after planting. In most of the cases, the first treatment (BZR 336g) generates the lower disease incidence and the second (BZR 517) is an intermediate value, when compared to the control. The only difference is in the cultivar Svitanok kievski, in 2015, when 10 weeks after plating it had no significant difference between the two treatments.  
...
The Duncan and Tukey tests did not have differences (Figures 9 and 10) and they had only few differences to the Kruskal test (Figure 11). The Shapiro-Wilk normality test was performed for Anova and Square Root Anova because in Anova p was lower than 0.05, what means that the data don't deviate from a normal distribution (Figure 12) and, in this case, the best test to be use is a nonparametric test, like the Duncan or Kruskal, to compare the differences between the treatments.

Taking into account the Kruskal test for disease severity, we can see a big difference between the control and the treatments principally 10 weeks after planting. In all cases, at this time, the first treatment (BZR 336g) generates the lower disease severity and the second (BZR 517) is an intermediate value, when compared to the control. 
...
We can see only few differences between the results of Duncan, Tukey and Kruskal test, in the differences between treatments (Figures 13, 14 and 15). The Shapiro-Wilk normality test was performed for Anova and Square Root Anova because in Anova p was lower than 0.05, that means that the data don't deviate from a normal distribution (Figure 16) and, in this case, the best test to be used is a nonparametric test, like the Duncan or Kruskal, to compare the differences between the treatments.

Taking into account the Kruskal test for number of stems, we can see that the differences are bigger between the treatments than between the weeks. In almost all the cases, the first treatment (BZR 336g) generates the biggest stems and the second (BZR 517) is an intermediate value, when compared to the control. We can also see that the cultivar Yuna has normally lower values of stems than the cultivar Svitanok kievski. We can also see that only the cultivar Yuna in 6 and 10 weeks after planting had no significant difference between treatments.
...
We can't see differences between the results of Duncan, Tukey and Kruskal test, in the differences between treatments (Figures 17, 18 and 19). The Shapiro-Wilk normality test was performed for Anova and Square Root Anova because in Anova p was higher than 0.05, what means that the data follow a normal distribution (Figure 20) and, in this case, the best test to be used is the Tukey test, to compare the differences between the treatments.

Taking into account the Tukey test for the biomass distribution, we can see that we have more biomass in the medium fraction and also that the efficiency of the treatments changes depending on the fraction. In this way we can see that, for the large fraction, the treatment BZR 336g results in the higher biomass production, however when we analyze the small fractions, the higher production of biomass takes place in the control. For the medium fractions, the treatment BZR 517 proved to be the most effective, however, there was no significant difference between the control for the cultivar Yuna.
...
We can't see differences between the results of Duncan, Tukey and Kruskal test, in the differences between treatments (Figures 21, 22 and 23). The Shapiro-Wilk normality test was performed for Anova and Square Root Anova because in Anova p was higher than 0.05, what means that the data follow a normal distribution (Figure 24) and, in this case, the best test to be used is the Tukey test, to compare the differences between the treatments.

Taking into account the Tukey test for the yield of potato, first, we can see that the both cultivars produced more in 2015 than in 2016. In the case of the cultivar Svitanok kievski, the treatment BZR 336g proved to be the most efficient, and the treatment BZR 517 an intermediary when compared to the control. In the case of the Yuna cultivar, there was no statistical difference between the two treatments, however both were more efficient when compared to the control.
...
It can be considered that our data sample is relatively small (1 plant per plot, with 5 repetitions). For potato crop evaluations, some studies recommend smaller plots with a greater number of repetitions in the same total area result in greater precision (Ribeiro et al, 2005). Also Ribeiro et al (2005) estimated an optimum plot size of (4) mounds per treatment for potato yield crop, and a great number or repetitions. Muzhinji et al (2018) recommend to sample 10 plants per plot/treatment with 4 repetitions for evaluating *R. solani* in potato crop. 

From the observation that some data sets do not follow a normal distribution, it can be inferred that the authors performed non parametric Duncan test, since this can be applied in the same conditions of the Tukey's test, but with a significance less cautionary. Also in the argument that a more liberal procedure is appropriate because in real world practice the global null hypothesis Ho = "All means are equal" is often false. Due to the lack of robustness of Duncan's test there are some cautions to follow, as many times it cannot be accepted as a proof of differences.

We performed Tukey (parametric) and Kruskal-Wallis (non parametric) in addition to Duncan test, to provide more robust analyzes. Tukey, is more conservative and therefore "less objectionable". When ANOVA assumptions are not met, it is recommended to perform Kruskal-Wallis test, which is an alternative to ANOVA when there are more than two groups to compare. Also Kruskal-Wallis test tends to be more robust and therefore more accepted in scientific publications.

The studied strains (BZR 336 g and BZR 517) were effective reducing the incidence and severity of *R. solani* on potato plants under yield conditions. *B. velezensis* strains improved plant height, number of stems per plant and increased the yield. Also the use of these strains improve the % of medium and large tubers. *B. velezensis* strains had an inhibitory effect in the *R. solani* colony growth under laboratory conditions. For yield trials, the strain BZR 336 g showed the best performance, while high concentrations of both strains performed the best inhibitory effect in laboratory.

It will be interesting to compare the B. velezensis effect with the performance of the current R. solani disease management.

## References 

Muzhinji, N., Woodhall, j. W., Truter, M., and Van der Waals, J. E. (2017). Relative Contribution of Seed Tuber- and Soilborne Inoculum to Potato Disease Development and Changes in the Population Genetic Structure of Rhizoctonia solani AG 3-PT under Field Conditions in South Africa. https://doi.org/10.1094/PDIS-03-17-0329-RE

Ribeiro de Oliveira, S. J., Storck, L. Sidinei, J. L., A., Dal'Col, L. A., Feijó, S. and Perin, D. H. (2005). Plot size and experimental unit relationship in exploratory experiments. https://doi.org/10.1590/S0103-90162005000600012
