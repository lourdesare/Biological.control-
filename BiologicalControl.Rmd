...
We can see only few differences between the results of Ducan, Tukey and Kruskal test, in the difference between treatments (Figures 1, 2 and 3). Also, the Shapiro-Wilk normality test was performed for Anova and Square Root Anova because in Anova p was lower than 0.05, that means that the data significantly deviate from a normal distribution (Figure 4) and, in this case the best test to be used would be the Tukey test because of the normal distribuction of the results.

Taking into account the plant height, we can see that the values increase over the weeks, however the difference between the treatments is not so great and in most cases the first treatment (BZR 336g) generates the highest plant height and the second (BZR 517) is an intermediate value, when compared to the control. Moreover this difference is most noticeable in week 10 when only the cultivar Svitanok kievski in 2016 had no significant difference between treatments. 
